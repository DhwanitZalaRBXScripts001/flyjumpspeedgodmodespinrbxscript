...
-- Specialty Utility Buttons (Small, side-by-side)
...
local btnWall = buildSmallBtn("WALL", UDim2.new(1, -70, 0, 15), Color3.fromRGB(200, 50, 255)) -- Purple to distinguish wall hack
...
-- ==========================================
-- ESP SYSTEM ENGINE (2D TRACERS & LABELS)
-- ==========================================
local espObjects = {}
...
-- ==========================================
-- CORE SCRIPTS (FLYING, WALLHACK, ESP RENDER)
-- ==========================================
...
-- WALL HACK TOGGLE LOGIC
btnWall.MouseButton1Click:Connect(function()
    Config.noclip = not Config.noclip
    if Config.noclip then
        tweenUI(btnWall, {BackgroundColor3 = Color3.fromRGB(150, 0, 200)}, 0.2) -- Glow purple when active
    else
        tweenUI(btnWall, {BackgroundColor3 = Color3.fromRGB(15, 15, 15)}, 0.2)
    end
end)

-- WALL HACK ENGINE (RunService.Stepped runs exactly before collision detection)
...
-- Heartbeat checks run EVERY FRAME to strictly enforce settings and draw ESP
...
player.CharacterAdded:Connect(function(char)
    Config.flying = false
    Config.noclip = false -- Reset wallhack on death to prevent glitching into floor
